...
[stem]
++++
P(t)=(t)/(sum_(i=1)^|T| t_i), T={t_1,...,t_(n in NN)}
++++
...
[cols="1,1,1,1",id="tbl-apache-jmeter-parameters-matrix"]
|===
h| ID
h| Number of Threads
h| Ramp-up Period
...
The variable stem:[i] is defined as an element of the set stem:[{B, C, N}].
Variables stem:[p], stem:[q], and stem:[r] are each defined as elements of the setstem:[{1,10,30}].